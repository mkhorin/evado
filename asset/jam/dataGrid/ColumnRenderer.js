...
Jam.ColumnRenderer = class ColumnRenderer {
    ...
    getFormatMethod (name) {
        return FORMAT_VALUE_METHODS.hasOwnProperty(name) 
            ? this[FORMAT_VALUE_METHODS[name]] 
            : this.asDefault;
    }
    ...
    translate (value, column) {
        return typeof column.translateData === 'string'
            ? Jam.t(value, column.translateData)
            : value;
    }
    ...
    join (handler, value, column, data) {
        ...
        let separator = '<br>';
        if (column.format?.hasOwnProperty('separator')) {
