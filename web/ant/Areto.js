...
class Ant {
    ...
    static postAction ($btn, params) {
        return Ant.Helper.confirm($btn.data('confirm'))
            ? $.post($btn.data('url'), params)
            : $.Deferred().reject();
    }
...
Ant.Resource = class {
    ...
    constructor () {
        ...
        $(document.head).find('link').each((index, element)=> {
            this.add(element.getAttribute("href"));
        });
        $(document.body).find('script').each((index, element)=> {
            this.add(element.getAttribute("src"));
        });
    }
