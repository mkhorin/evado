...
module.exports = class CrudController extends Base {
    ...
    async actionRemoveList () {
        ...
        let models = await this.getModelClass().findById(ids.split(',')).all();
        let ModelClass = await this.getModelClass();
    ...
    actionList (query, params) {
        query = query || this.getModelClass().find();
