...
module.exports = class MetaTransitionInspector extends Base {
    ...
    static concatHierarchyItems (items, map) {
        ...
        for (let item of items) {
            if (item.project && !item.class) {
                Rbac.concatFirstArrayItems(item.key, map, ALL);
            }
        }
        for (let item of items) {
            if (!item.transition && !item.object && item.class) {
                Rbac.concatFirstArrayItems(item.key, map, `...${item.project}`, ALL);
            }
        }
        for (let item of items) {
            if (!item.transition && item.object) {
                Rbac.concatFirstArrayItems(item.key, map, `..${item.class}.${item.project}`, `...${item.project}`, ALL);
            }
        }
        for (let item of items) {
            if (item.transition && item.object) {
                Rbac.concatFirstArrayItems(item.key, map, `${item.transition}..${item.class}.${item.project}`);
            }
        }
        return map;
    }
    ...
    async execute () {
        ...
        let items = this.itemMap[this.objectKey]
            ...
            || this.itemMap[`...${this.target.class.project.id}`]
            || this.itemMap[`...`];
    ...
    async resolveTransitions () {
        if (this.transitions instanceof Array) {
